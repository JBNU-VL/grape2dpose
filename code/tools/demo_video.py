...
def show_crowdpose_skeleton(img, kpts, thr=0.2):
    kpts = np.array(kpts).reshape(-1,3)
    skelenton = [[0, 2], [1, 3], [2, 4], [3, 5], [6, 8], [8, 10], [7, 9], [9, 11], [12, 13], [0, 13], [1, 13], [6,13],[7, 13]]
    skeleton_color = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (255, 165, 0), (255, 192, 203),
                      (0, 128, 128), (128, 0, 128), (128, 128, 0),
                      (255, 0, 255), (255, 215, 0), (210, 105, 30), (70, 130, 180), (255, 99, 71), (240, 128, 128),
                      (50, 205, 50), (165, 42, 42), (255, 255, 255), (255, 128, 128)]
...
def main():
    ...
    MATCH_HMP = True
    vidcap = cv2.VideoCapture("../../demo_images_video/video/demo_video.mp4")
    fps = vidcap.get(cv2.CAP_PROP_FPS)
    if fps < 10:
        raise ValueError('desired inference fps is ' + str(10) + ' but video fps is ' + str(fps))

    frame_width = int(vidcap.get(cv2.CAP_PROP_FRAME_WIDTH))
    frame_height = int(vidcap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    outcap = cv2.VideoWriter(
        '{}/{}_pose.avi'.format('../../demo_images_video/video/', os.path.splitext(os.path.basename("../../demo_images_video/video/demo_video.mp4"))[0]),
        cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30, (frame_width, frame_height))
    outcap_black = cv2.VideoWriter(
        '{}/{}_black.avi'.format('../../demo_images_video/video/', os.path.splitext(os.path.basename("../../demo_images_video/video/demo_video.mp4"))[0]),
        cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 30, (frame_width, frame_height))

    count = 0
    while vidcap.isOpened():
        ret, image_rgb = vidcap.read()
        count += 1
        ...
        image_black = np.zeros_like(image_pose)

        base_size, center, scale = get_multi_scale_size(image_rgb, cfg.DATASET.INPUT_SIZE, 1.0, 1.0)
        ...
        with torch.no_grad():
            ...
            for scale in sorted(scales, reverse=True):
                image_resized, center, scale_resized = resize_align_multi_scale(image_rgb, cfg.DATASET.INPUT_SIZE, scale, 1.0)
            ...
            for coords in final_results:
                imgshow = show_crowdpose_skeleton(image_pose, coords)
                imgblack = show_crowdpose_skeleton(image_black, coords)

        outcap.write(imgshow)
        outcap_black.write(imgblack)

    vidcap.release()
    outcap.release()
    outcap_black.release()
