...
def absorb_heat(cfg, poses, heatmap_avg):
    ...
    for i in range(num_joints):
        ...
        for p in range(poses.shape[0]):
            if pose_score[p] > cfg.TEST.VIS_THRE:
                poses[p, i, :-2] = heat_ind[keep_ind[p]]

    return [poses[:, :, :3].cpu().numpy()]
...
def fit_gaussian_heatmap(func, heatmap, maxval, init_y, init_x, sigma):
    ...
    popt, _ = curve_fit(func, x_data, heatmap.ravel(), p0=init_guess, maxfev=300)
