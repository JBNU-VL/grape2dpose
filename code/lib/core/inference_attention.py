...
def get_multi_stage_outputs(cfg, model, image, with_flip=False):
    heatmap, _, offset, visibilitymap, _ = model(image)   # 原始的bao的架构
    # heatmap, offset, _, visibilitymap, _ = model(image)   # 原始的bao的架构

    visibilitymap = torch.sigmoid(visibilitymap)
    ...
    if with_flip:
        if 'coco' in cfg.DATASET.DATASET:
            ...
            flip_index_visibility = FLIP_CONFIG['COCO_VISIBILITY']
            ...
            flip_index_visibility = FLIP_CONFIG['CROWDPOSE_VISIBILITY']
        else:
        ...
        image[:, :, :, :-3] = image[:, :, :, 3:]
        heatmap_flip, _, offset_flip, visibilitymap_flip, _ = model(image)
        # heatmap_flip, offset_flip, _, visibilitymap_flip, _ = model(image)


        visibilitymap_flip = torch.sigmoid(visibilitymap_flip)
        ...
        visibilitymap_flip = torch.flip(visibilitymap_flip, [3])
        visibilitymap = (visibilitymap + visibilitymap_flip[:, flip_index_visibility, :, :]) / 2.0

    return heatmap, posemap, visibilitymap
...
def aggregate_results(cfg, heatmap_sum, visibilitymap_sum, poses, heatmap, posemap, visibilitymap, scale):
    """
    ...
    visibilitymap_sum += up_interpolate(
        visibilitymap,
        size=(int(reverse_scale * w), int(reverse_scale * h)),
        mode='bilinear'
    )
    ...
    return heatmap_sum, poses, visibilitymap_sum
