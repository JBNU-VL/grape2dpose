...
class OchumanDataset(Dataset):
    ...
    def _get_anno_file_name(self):
        dataset = 'val' if 'rescore' in self.dataset else self.dataset
        return os.path.join(
            self.root,
            'annotations',
            'ochuman_{}.json'.format(
                dataset
            )
        )
    ...
    def evaluate(self, cfg, preds, scores, output_dir, tag, *args, **kwargs):
        '''
        ...
        res_file = os.path.join(res_folder, 'keypoints_%s_results.json' % (self.dataset + tag))
...
class OchumanRescoreDataset(OchumanDataset):
    def __init__(self, cfg, dataset):
        OchumanDataset.__init__(self, cfg, dataset)
