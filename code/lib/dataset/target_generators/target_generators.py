...
class OffsetGenerator():
    ...
    def __call__(self, joints, area):
        ...
        offset_map = np.zeros((self.num_joints_without_center * 2, self.output_h, self.output_w), dtype=np.float32)
        weight_map = np.zeros((self.num_joints_without_center * 2, self.output_h, self.output_w), dtype=np.float32)
        area_map = np.zeros((self.output_h, self.output_w), dtype=np.float32)
        visibility_map = np.zeros((self.num_joints_without_center, self.output_h, self.output_w), dtype=np.float32)
        ...
        for person_id, p in enumerate(joints):
            ...
            for idx, pt in enumerate(p[:-1]):
                if pt[2] > 0:
                    ...
                    #                             visibility_map[idx, pos_y, pos_x] = 1.

                    start_x_vis = max(int(x - self.radius_visibility), 0)
                    start_y_vis = max(int(y - self.radius_visibility), 0)
                    end_x_vis = min(int(x + self.radius_visibility), self.output_w)
                    end_y_vis = min(int(y + self.radius_visibility), self.output_h)

                    for pos_x in range(start_x_vis, end_x_vis):
                        for pos_y in range(start_y_vis, end_y_vis):
                            visibility_map[idx, pos_y, pos_x] = 1.

        return offset_map, weight_map, visibility_map
