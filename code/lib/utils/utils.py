...
def get_optimizer_frozen(cfg, model, list_layer_frozen):
    optimizer = None
    if cfg.TRAIN.OPTIMIZER == 'sgd':
        optimizer = optim.SGD(
            model.parameters(),
        ...
        pretrained_params = list(
            map(lambda x: x[1], list(filter(lambda kv: kv[0] in list_layer_frozen, model.named_parameters()))))
        not_pretrained_params = list(
            map(lambda x: x[1], list(filter(lambda kv: kv[0] not in list_layer_frozen, model.named_parameters()))))
        optimizer = torch.optim.Adam([{'params': filter(lambda p: p.requires_grad, pretrained_params)},
                                      {'params': filter(lambda p: p.requires_grad, not_pretrained_params),
                                       'lr': cfg.TRAIN.LR}], lr=cfg.TRAIN.LR_BASE)
        ...
        not_pretrained_params = list(
            map(lambda x: x[1], list(filter(lambda kv: kv[0] not in list_layer_frozen, model.named_parameters()))))
        optimizer = torch.optim.AdamW([{'params': filter(lambda p: p.requires_grad, pretrained_params)},
                                       {'params': filter(lambda p: p.requires_grad, not_pretrained_params),
                                        'lr': cfg.TRAIN.LR}], lr=cfg.TRAIN.LR_BASE, weight_decay=0.0001)
